...
class LinearNormal(nn.Module):
    def __init__(self, in_chanels, out_channels):
        super().__init__()
        self.linear = nn.Linear(in_chanels, out_channels * 2)
    
    def forward(self, x):
        pred = self.linear(x).view(x.shape[0], -1, 2)
        mu, v = [w.squeeze(-1) for w in torch.split(pred, 1, dim=-1)]
        return mu, torch.nn.Softplus()(v) + 1e-6
...
class update_u(torch.nn.Module):
    def __init__(self, hidden_channels, uncertainty):
        ...
        if self.uncertainty == 'False':
            ...
            self.lin2 = LinearNormalGamma(hidden_channels //2, 1)
...
class SchNet(torch.nn.Module):
    ...
    def __init__(self, energy_and_force=False, cutoff=10.0, num_layers=6, hidden_channels=128, num_filters=128, num_gaussians=50, noise_var = 0.001, uncertainty = 'False'):
        ...
        self.update_u = update_u(hidden_channels, uncertainty)
    ...
    def forward(self, batch_data, add_pos_noise = False):
        z, pos, batch = batch_data.z, batch_data.pos, batch_data.batch
        
        if add_pos_noise:
            pos = pos + torch.empty(pos.shape).normal_(mean=0,std=math.sqrt(self.noise_var)).to(device)
